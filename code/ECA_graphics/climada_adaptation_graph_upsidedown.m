...
function  fig = climada_adaptation_graph_upsidedown(ECA_adaptation, case_i, digits)
...
color_     = [color_adaptation;...% cost-efficient measures
...
if ~ex_flag
    for m_i = 1:no_measures-1
        ...
        text(mean(cum_benefit([m_i m_i+1])),max([BenefitCost; ymax_limit])-1.5,...
             Measure{m_i}, 'Rotation',90,'FontSize',fontsize_2+1, 'color', color_text(c_index(m_i),:),... %'FontName','Helvetica-Narrow',
...
% 20160428, muellele@gmail.com, works for version 2015
length_factor = 3.2;
if cum_benefit(end)>cum_benefit(m_cost_eff)
    climada_arrow([cum_benefit(end) y_*1.0], [cum_benefit(m_cost_eff)+s_/2 y_*1.0],...
        'width',25,'Length',20*length_factor, 'BaseAngle',90, 'EdgeColor','none', 'FaceColor',color_(3,:)); 
...
if cum_benefit(end-1)>cum_benefit(m_cost_eff)
    climada_arrow([cum_benefit(m_cost_eff)+s_*3 y_], [cum_benefit(end-1)-s_/2 y_], ...
        'width',15,'Length',13*length_factor, 'BaseAngle',90, 'EdgeColor','none', 'FaceColor',color_(2,:)); 
         %'width',15,'Length',13, 'BaseAngle',90, 'TipAngle', 130,'EdgeColor','none', 'FaceColor',color_(2,:));
end
climada_arrow([0 y_], [cum_benefit(m_cost_eff) y_],'width',25,'Length',20*length_factor, 'BaseAngle',90, 'EdgeColor','none', 'FaceColor',color_1);%color_(1,:));
...
if a_cost_eff >ymax_limit/80 %> 0.25
    text(cum_benefit(m_cost_eff)/2                     ,y_,'Cost-efficient adaptation','color','k','HorizontalAlignment','center','VerticalAlignment','middle','fontsize',fontsize_2,'fontweight','bold');
elseif a_cost_eff >ymax_limit/133 %> 0.15
    text(cum_benefit(m_cost_eff)/2                     ,y_,'Cost-efficient adaptation','color','k','HorizontalAlignment','center','VerticalAlignment','middle','fontsize',fontsize_2-2,'fontweight','bold');
end
if a_non_cost >ymax_limit/133 %> 0.15
    text((cum_benefit(m_cost_eff)+cum_benefit(end-1))/2,y_,'Non-cost-efficient'       ,'color','k','HorizontalAlignment','center','VerticalAlignment','middle','fontsize',fontsize_2,'fontweight','bold');
elseif a_non_cost >ymax_limit/82 %> 0.11
    text((cum_benefit(m_cost_eff)+cum_benefit(end-1))/2,y_,'Non-cost-efficient'       ,'color','k','HorizontalAlignment','center','VerticalAlignment','middle','fontsize',fontsize_2-5,'fontweight','bold');
end
if a_residual >ymax_limit/133 %> 0.15
    text(mean(cum_benefit([end end-1]))                ,y_,'Residual loss 2030'       ,'color','w','HorizontalAlignment','center','VerticalAlignment','middle','fontsize',fontsize_2-2,'fontweight','bold');
    %text(mean(cum_benefit([end end-1]))                ,y_,'Residual loss 2030'       ,'color','w','HorizontalAlignment','center','VerticalAlignment','middle','fontsize',fontsize_2-2,'fontweight','bold');
elseif a_residual >ymax_limit/285 %> 0.07
    text(mean(cum_benefit([end end-1]))                ,y_,'Residual loss 2030'       ,'color','w','HorizontalAlignment','center','VerticalAlignment','middle','fontsize',fontsize_2-8,'fontweight','bold');
    %text(mean(cum_benefit([end end-1]))                ,y_,'Residual loss 2030'       ,'color','w','HorizontalAlignment','center','VerticalAlignment','middle','fontsize',fontsize_2-6,'fontweight','bold');
end
...
%title
ylabelstr = 'Reduced loss per USD invested (USD)';
...
if ~ex_flag
    if bn_flag == 1
        xstr = 'Averted loss (bn USD)';
    else
        xstr = 'Averted loss (mn USD)';
    end
else
    xstr = 'Averted loss (mn USD)';
end
